...
#-------------------------------------------------------------------------------
    if kernel_rv[0:2] == 'VR' or kernel_rv[0:3] == 'R15' or kernel_rv[0:2] == 'MQ':
        ...
        m, C =pti.pred_gp(kernel_rv,pk_rv,xvec,yvec,evec,rvx_tot,jrv,jrvlab)
        #Let us remove the GP model from the data
        m_gp, C_gp =pti.pred_gp(kernel_rv,pk_rv,xvec,yvec,evec,xvec,jrv,jrvlab)
        #-----------------------------------------------------------------------------------
        ...
        plot_vector[0] = [rvx,(rvy)*cfactor,m[0:nts]*cfactor,(rvy+m[0:nts])*cfactor]
        for i in range(1,ns):
    ...
    if kernel_rv[0:2] == 'VR' or kernel_rv[0:3] == 'R15' or kernel_rv[0:2] == 'MQ':
      ...
      for o in range(0,ns):
        rv_dvec = [xdata[o*ts_len:(o+1)*ts_len],ydata[o*ts_len:(o+1)*ts_len]\
                  ,edata[o*ts_len:(o+1)*ts_len],ejdata[o*ts_len:(o+1)*ts_len]\
                  ,yvec_noplanet[o*ts_len:(o+1)*ts_len]*cfactor,tlab[o*ts_len:(o+1)*ts_len]]
        rv_dvecnp = np.asarray(rv_dvec)
        mvec = plot_vector[o]
        mvecnp = np.asarray(mvec)
        np.savetxt(outdir+'/timeseries_model'+str(o)+'.dat',mvecnp.transpose())
        np.savetxt(outdir+'/timeseries_data'+str(o)+'.dat',rv_dvecnp.transpose())
        if o == 0:
          model_labels = ['Planetary signal','GP','P+GP']
          mcolors = ['r','b','k']
          malpha = [0.7,0.7,0.9]
        ...
        if ( len(rv_labels) == 1 ) :
            ...
            plot_labels_rv = [rv_xlabel[o*ns:(o+1)*ns],rv_labels[o],rv_res[o]]
    ...
    for i in range(0,nplanets):
      ...
      for m in range(0,len(mega_rv)):
        res[m] = mega_rv[m] - v_val[tlab[m]] - rv_pall[m]  \
                 - alpha_val*(mega_time[m] - t0_val[0]) - beta_val*(mega_time[m] - t0_val[0])**2
...
#      plot_rv_fancy([p_rv,rvy,p_all,rv_planet_i,evec,ejvec,res,tlab],fname)
      rv_dvec = [p_all,rv_planet_i,evec,ejvec,res,tlab]
      tellabs = telescopes_labels
      if kernel_rv[0:2] == 'VR' or kernel_rv[0:3] == 'R15' or kernel_rv[0:2] == 'MQ':
        ns = int((len(fit_krv) - 3)/2)
        nd = int(len(p_all)/ns)
        rv_dvec = [p_all[0:nd],rv_planet_i[0:nd],evec[0:nd],ejvec[0:nd],res[0:nd],tlab[0:nd]]
        tellabs = [0]
        tellabs[0] = telescopes_labels[0]
      rv_mvec = [p_rv,rvy]
      plot_labels_rv = ['Orbital phase','RV (m/s)','Residuals (m/s)']
      model_labels = ['']
      create_nice_plot(rv_mvec,rv_dvec,plot_labels_rv,model_labels,tellabs,fname)
