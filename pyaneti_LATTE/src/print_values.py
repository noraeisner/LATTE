...
#Print the summary
  ...
  var  = ['M_*', 'R_*', 'T_*']
  val  = [mstar_mean, rstar_mean,tstar_mean]
  pos  = [mstar_sigma,rstar_sigma,tstar_sigma]
  neg  = [mstar_sigma,rstar_sigma,tstar_sigma]
  unit = ['solar mass', 'solar radii', 'K']

  # write the header information 
  metadata_header = ["Var", "Val", "Pos", "Neg", "Unit"]
  with open('{}/{}_parameters.csv'.format(outdir, tic), 'w') as f: # save in the photometry folder
      writer = csv.writer(f, delimiter=',')
      writer.writerow(metadata_header)

  # write the file information
  with open('{}/{}_parameters.csv'.format(outdir, tic), 'a') as f: # save in the photometry folder
      writer = csv.writer(f, delimiter=',')
      for i,v in enumerate(var):
          metadata_data = [v]
          metadata_data.append(val[i])
          metadata_data.append(pos[i])
          metadata_data.append(neg[i])
          metadata_data.append(unit[i])
  ...
  for o in range(0,nplanets):
    ...
    for m in range(0,nradius):
      rr_vec[o*nradius+m] = list(np.asarray(list(params[4+srp+nradius*o+m])))
...
#STARTING CALCULATIONS
  ...
  #Transit durations aproximations (eq. 14, 15, 16 from Winn 2014)
    ...
    jsp = k_btz * np.asarray(Teq_vec[o]) * S_radius_cgs * np.asarray(r_vec[o*nradius])
    ...
    pgra_vec = (P_vec[o]*24.*3600.) * (rr_vec[o*nradius]/ar_vec[o])**2 * np.sin(i_vec[o])
...
#    msgra = msgra / S_GM_cgs * G_cgs
    ...
    if ( unit_mass == 'earth'):
      ...
      if ( fit_tr[o] ):
        for m in range(0,nradius):
          r_vec[o*nradius+m] = r_vec[o*nradius+m] * S_radius_SI / E_radius_e_SI
      ...
      if ( fit_tr[o] ):
        for m in range(0,nradius):
          r_vec[o*nradius+m] = r_vec[o*nradius+m] * S_radius_SI / J_radius_e_SI
    ...
    if ( np.mean (r_vec[o*nradius]) < 1.5 ):
      ...
      TSM = 1.15
    #
    TSM = TSM * r_vec[o*nradius]**3*Teq_vec[o] / m_vec[o] / rstar**2 * 10.**(-mag_j/5.)
    ...
    if ( fit_tr[o] ):
      ...
      for m in range(0,nradius):
        print_values(rr_vec[o*nradius+m],'rp/R*'+bands[m],'rr'+pl+bands[m],' ',' ')
    if ( fit_rv[o] ):
    ...
    if (fit_tr[o]):
      for m in range(0,nradius):
        print_values(r_vec[o*nradius+m],'Rp'+bands[m],'rp'+pl+bands[m],'R_'+unit_mass,'$R_'+usymbol+'$')
    ...
    if ( fit_tr[o]):
      ...
      for m in range(0,nradius):
        print_values(1.e6*rr_vec[o*nradius+m]**2,'depth'+bands[m],'depth'+pl+bands[m],'ppm','ppm')
        print_values((np.sqrt(1.-b_vec[o]**2)*rr_vec[o*nradius+m]**2*vsini_mean)*1.e3,'RM'+bands[m],'RM'+pl+bands[m],'m/s','${\\rm m\,s^{-1}}$')
      print_values(Fp,'Insolation','insolation'+pl,'F_Earth','${\\rm F_{\\oplus}}$')
      print_values(jsp,'Jeans','jsp'+pl,'Jeans Scape Parameter (Fossati et al 2017)',' ')
 ...
 #     print_values(msgra,'M_*','mspars'+pl,' solar masses (scaled parameters)','$M_\odot$')
      ...
      print_values((trt_vec[o] - tri_vec[o])/2.0,'T_in/eg','teg'+pl,'hours','hours')
      if is_single_transit:
          print_values(tr_dur_vec*24.,'T_duration','t_dur'+pl,'hours','hours')
...
if ( total_rv_fit ):
  for o in range(0,nt):
    print_values(rv_vec[o],'Sys. vel. '+telescopes_labels[o],telescopes_labels[o],'km/s','${\\rm km\,s^{-1}}$')
opars.write ('--------------------------------------------------------------\n')
