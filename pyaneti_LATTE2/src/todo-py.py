  ...
  if ( total_rv_fit ):
    ...
    if fit_v0.__class__ == str:
      ...
      for o in range(0,nt):
        if (fit_v0_val == 'u'):
          fit_v0[o] = 'u'
          min_rv0[o] = min(rv_all[o]) - 1.
          max_rv0[o] = max(rv_all[o]) + 1.
...
#vals do we want to print median or mode
  ...
  with open('{}/{}_parameters.csv'.format(outdir, tic), 'a') as f: # save in the photometry folder
      ...
      metadata_data = [var]
      metadata_data.append(medv)
      metadata_data.append(minv)
      metadata_data.append(maxv)
      metadata_data.append(unit)
...
def joint_fit():
  ...
  if is_jitter_rv:
      ...
      jrv_prior_vals = [1e-4,1.e-3]*n_jrv
  ...
  val_linear = 0.1
  val_quadratic = 0.1
  if ( is_linear_trend == 'f' ): val_linear = 0.0
  if ( is_quadratic_trend == 'f' ): val_quadratic = 0.0
  ...
  if kernel_rv == 'None':
     ...
     krv_labels = ['A','$\lambda_e$','$\lambda_p$','P']
  elif kernel_rv == 'QPK':
     krv_prior_flag = fit_krv #fit_krv has to be a four-dimensional vector (A,Gamma1,Gamma2,P)
     krv_prior_vals = krv_priors #This has to be a 4-dimensional vector with the prior limits
     krv_labels = ['A','$\lambda_e$','$\lambda_p$','P']
  ...
  model_int = [nplanets,nt,nbands,nradius,nldc,n_jrv,n_jtr,np_rv,np_tr]
