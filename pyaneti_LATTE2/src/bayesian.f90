...
!Local variables
  ...
  if (tff(0) ) then
    ...
    else if (kernel_rv == 'VR2' ) then

    !This is the V. Rajpaul Framework
    res_rv = 0.0
    call find_res_rv(x_rv(0:size_rv/mkernel-1),y_rv(0:size_rv/mkernel-1),rvlab(0:size_rv/mkernel-1),&
                     pars_rv,flag_rv,res_rv(0:size_rv/mkernel-1),size_rv/mkernel,ntels,npl)
    do i = 1, mkernel - 1
      res_rv(i*size_rv/mkernel:(i+1)*size_rv/mkernel-1) = y_rv(i*size_rv/mkernel:(i+1)*size_rv/mkernel-1) &
                                                        - pars(srv+rvlab(i*size_rv/mkernel:(i+1)*size_rv/mkernel-1))
