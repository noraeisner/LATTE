      ...
      for m in range(0,nbands):
        ...
        for n in range(0,len(megax)):
          if ( trlab[n] == m ):
              ...
              localt.append(0)
        tr_vector[m] = create_tr_vector(localx,localy,locale,localt,pars_tr,rp_val,o,m)
...
def fancy_tr_plot(tr_vector,pnumber):
  ...
  gs = gridspec.GridSpec(nrows=2, ncols=1, height_ratios=[3.0, 1./(1+(nbands-1)*0.75)])
  ...
  for m in range(0,nbands):
    if ( plot_tr_errorbars  ):
      ...
      plt.plot((xtime[m]-local_T0)*tfc,yflux[m]-deltay,color=tr_colors[m],ms=7,marker=mark_tr[m],alpha=0.75,linewidth=0.)
      ...
      plt.annotate(bands[m],xy=(-x_lim*(0.70),min(yflux[m]-deltay)),fontsize=fos*0.7)
      if ( m < nbands - 1): dy = max(yflux[m+1])-min(yflux[m+1])
      deltay = deltay + dy
      if plot_binned_data: plt.plot(xbined,fbined,'ro')

      #save the data
      model_matrxi = np.asarray([(xmodel[m]-local_T0)*tfc,fd_ub[m]])
      np.savetxt(outdir+'/'+star+'-trmodel'+bands[m]+'.txt',model_matrxi.T)
      data_matrxi = np.asarray([(xtime[m]-local_T0)*tfc,yflux[m],eflux[m],res_res[m]])
      np.savetxt(outdir+'/'+star+'-trdata'+bands[m]+'.txt',data_matrxi.T)
      #if ( is_special ):
...
#  plt.xticks( np.arange(int(x_lim),int(-x_lim)+1,step_plot))
  ...
  if plot_binned_data: plt.plot(xbined,rbined*1e6,'ro')
  #Calcualte the rms
  if ( is_plot_std_tr ):
...
def create_tr_vector(time,flujo,eflujo,trlab,pars_tr,rp,plabel,bandlab):
    ...
    fd_ub = pti.flux_tr(xmodel,newtrlab,dparstr,rp_val[plabel*nradius+bandlab*control],my_ldc[bandlab*2:bandlab*2+2],n_cad[bandlab],t_cad[bandlab],nradius=1)
    #Let us create an unbinned model plot
    #fd_ub_unbinned = pti.flux_tr(xmodel,dparstr,my_ldc,1,t_cad)
    fd_ub_unbinned = pti.flux_tr(xmodel,newtrlab,dparstr,rp_val[plabel*nradius+bandlab*control],my_ldc[bandlab*2:bandlab*2+2],[1],t_cad[bandlab],nradius=1)
    ...
    fd_ub_res = pti.flux_tr(xmodel_res,newtrlab,dparstr,rp_val[plabel*nradius+bandlab*control],my_ldc[bandlab*2:bandlab*2+2],n_cad[bandlab],t_cad[bandlab],nradius=1)
   ...
   # Let us calculate the flux caused by the other planets
    ...
    if kernel_tr[0:2] != 'No':
        ...
        yvec = res_res
        evec = eflux
        m, C =pti.pred_gp(kernel_tr,pk_tr,xvec,yvec,evec,xvec,jtr[bandlab],[0]*len(xvec))
    ...
    if ( fit_tr[i] ):
      ...
      if ( is_plot_all_tr[i] ):
        for j in range(0,len(xt)):
          ...
          ytm = pti.flux_tr(xtm,trlab,pars_tr,rp_val,my_ldc,n_cad,t_cad)
...
def plot_lightcurve_timeseries(sigma=10):
    ...
    xmodel = np.arange(min(megax),max(megax),1./5./24.)
    my_trlab = [0]*len(xmodel)
    ymodel = pti.flux_tr(xmodel,my_trlab,pars_tr.transpose(),rp_val,my_ldc,n_cad,t_cad,nradius)
    #ymodel = pti.flux_tr(xmodel,my_trlab,pars_tr,rp_val,my_ldc,n_cad,t_cad)

    #Calcualte the residuals
    trres = pti.flux_tr(megax,trlab,pars_tr.transpose(),rp_val,my_ldc,n_cad,t_cad,nradius)
    trres = megay - trres
    ...
    if kernel_tr[0:2] != 'No':
        ...
        yvec = megay
        evec = megae
        m, C =pti.pred_gp(kernel_tr,pk_tr,xvec,trres,evec,xmodel,jtr,jtrlab)
