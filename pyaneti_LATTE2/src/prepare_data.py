...
if ( nplanets_rv > 0 ):
  ...
  time,rv,err,tspe = np.loadtxt(indir+'/'+tic+'/'+fname_rv[0],usecols=(0,1,2,3), \
  dtype={'names': ('time', 'rv', 'err','telescope'), \
    'formats': ('float', 'float', 'float', 'S10')}, \
    comments='#',unpack=True)
  #time,rv,err,tsp = np.genfromtxt('inpy/'+star+'/'+fname_rv[0], dtype=None, usecols=(0,1,2,3),unpack=True)
  if ( is_special_jitter ):
    sjitter = np.loadtxt(indir+star+'/'+fname_rv[0],usecols=(4),dtype=str,unpack=True)

  #Check for the correct coding
  #tl = [None]*len(tspe)
  #for m in range(len(tspe)):
  #    tl[m] = b(tspe[m])
  #tspe = tl
  ...
  if ( is_special_jitter ):
    ...
    for o in range(0,len(tlab)):
      for ndum in range(0,len(jrvvec)):
        if (b(sjitter[o][0]) == jrvvec[ndum][0] ):
          jrvlab.append(ndum)
          break
  else:
    jrvlab = list(tlab)
...
if ( nplanets_tr > 0 ):
  ...
  if (len(bands) == 1):
     ...
     nbands = len(bands)
     nradius = 1
     if is_multi_radius: nradius = nbands
