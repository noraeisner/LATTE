...
Other tools that are often used to extract and vet TESS light curves include the open source packeges *Lightkurve* [@2018lightkurve] and *eleanor* [@2019eleanor], for the long and short-cadence data respectively. These tools can perform similar diagnostic tests to the ones provided by ``LATTE``, however, ``LATTE`` differs in that it does not require any interaction with python coding, and summarizes multiple diagnostic tests in an automated manner. 
